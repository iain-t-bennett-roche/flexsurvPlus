...
variety of shapes depending on the distribution:[@RbloggersPSM]
...
understand which distribution might be the best fit to the data being modelled:
...
`1-WEIBULL(time,EXP(shape),EXP(scale),TRUE)`
...
`EXP((-1/shape) * EXP(rate) * (EXP(shape*time)-1))`
...
`1 /(1+(((time * EXP(-1 * scale))^(1/EXP(-1 * shape)))))`
...
`1-LOGNORMDIST(time,meanlog,1/EXP(-1*sdlog))`
...
`1-((EXP(GAMMALN(EXP(rate))) * GAMMA.DIST(EXP(shape) * time,EXP(rate),1,TRUE))/GAMMA(EXP(rate))`
...
`IF(Q<0,GAMMADIST((-Q^-2) * EXP(-Q* -((LN(time)-(mu))/EXP(sigma))),-Q^-2,1,1),1-GAMMADIST((-Q^-2) * EXP(-Q * -((LN(time)-(mu))/EXP(sigma))),-Q^-2,1,1))`
